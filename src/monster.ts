...
export class Monster extends Character {
  ...
  constructor(name: string = 'Monster', level: number = 1, currentPos: number[] = [randomPosition()[0], randomPosition()[1]]) {
  ...
  appear() {
    if (fixedWall.some((elem) => elem[0] === this.currentPos[0] && elem[1] === this.currentPos[1])) {
      ...
      const image = document.getElementById('skeleton') as HTMLImageElement;
